...
";
    ...
    function preprocess() {
        ...
        preg_match_all('@
            ...
            |
            (\s*\/\/\s*csscomb:\s*false[\r].*?\/\/\s*csscomb:\s*true[^\S\r]*)
            @ismx', $this->code['edited'],
            $this->code['magicComments'], PREG_SET_ORDER);
        foreach ($this->code['magicComments'] as $key => $value) {
            $this->code['edited'] = str_replace($value[0], "magic: comment".$key.'__;', $this->code['edited']);
        }
        // Comments
        preg_match_all('@
            ([\r]\s*/\*.*?\*/)+
    ...
    function resort_properties($prop) {
        ...
        if (is_array($this->sort_order[0])) {
            ...
            if (
                ...
                substr_count(substr($undefined[0], 0, 2), "") === 0
            ) {
    ...
    function postprocess() {
        ...
        if (is_array($this->code['magicComments'])) {
            foreach ($this->code['magicComments'] as $key => $val) {
                $this->code['resorted'] = str_replace("magic: comment".$key.'__;', $val[0], $this->code['resorted']);
            }
        }
    ...
    private function separate_property_group($properties) {
        if (is_array($this->sort_order[0])) { // Если в настройках нет разбиения на группы, то выводим входной массив без изменений
            foreach ($properties as $key => &$property) {
                $property = preg_replace('@\s*?(\s*?)@ismx', '$1', $property);
...
function init($argc, $argv){
    ...
    //echo "argc: ".$this->argc."";
    //echo "argv: "; var_dump($this->argv); echo "";
